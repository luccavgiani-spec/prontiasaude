...
# Código Completo: mp-create-payment v4

**BUILD_ID**: `mp-create-payment@2026-02-04T20:00:00Z-v4`

## Instruções de Deploy

1. Acesse: https://supabase.com/dashboard/project/ploqujuhpwutpcibedbr/functions/mp-create-payment
2. Clique em "Edit"
3. Substitua TODO o código pelo conteúdo abaixo
4. Clique em "Deploy"
...
interface PaymentRequest {
  ...
  issuer_id?: string; // ✅ NOVO: Código do banco emissor (+2 pontos qualidade)
  token?: string;
  installments?: number;
  metadata: {
    order_id: string;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[mp-create-payment] 🔒 SECURITY CHECKLIST PRE-VALIDATION:', {
      ...
      issuer_id: paymentRequest.issuer_id // ✅ NOVO: Log do issuer_id
    });

    // ✅ CORREÇÃO: Device ID - avisar se não foi capturado, mas nunca bloquear
    // O frontend agora captura explicitamente via window.MP_DEVICE_SESSION_ID
    if (!paymentRequest.device_id) {
      console.warn('[mp-create-payment] ⚠️ Device ID não recebido. Isso pode afetar taxa de aprovação.');
    }
    ...
    // ✅ CORREÇÃO v4: Remover city e federal_unit (API espera códigos IBGE numéricos)
    // ✅ CORREÇÃO v4: Usar city_name (não city) no receiver_address
    const fullAdditionalInfo = {
      ...
      payer: {
        ...
        address: {
          ...
          // ✅ REMOVIDO: city e federal_unit - API espera códigos IBGE numéricos, não strings
        },
      ...
      shipments: {
        receiver_address: {
          ...
          street_name: finalPayer.address?.street_name,
          street_number: finalPayer.address?.street_number,
          // ✅ CORREÇÃO: Usar city_name (não city) - campo correto da API
    ...
    if (isPix) {
      ...
      paymentData.issuer_id = paymentRequest.issuer_id; // ✅ NOVO: Código do emissor (+2 pontos qualidade)
    ...
    console.log('[mp-create-payment] Creating payment:', {
      ...
      issuer_id: paymentData.issuer_id || 'N/A' // ✅ NOVO: Log
    });

    // ✅ CORREÇÃO v4: Não enviar Device ID vazio - só logar warning
    if (!paymentRequest.device_id) {
      console.warn('[mp-create-payment] ⚠️ Device ID não recebido do frontend. Antifraude pode ter menor precisão.');
    }
    ...
    console.log('[mp-create-payment] 🚀 Using official Mercado Pago SDK (v4 - Device ID + issuer_id + additional_info fixes)');
    ...
    const debug_context = {
      ...
      has_device_id: !!paymentRequest.device_id, // ✅ NOVO
      has_issuer_id: !!paymentRequest.issuer_id // ✅ NOVO
    };
    ...
    // ✅ CORREÇÃO v4: Headers condicionais - só enviar X-meli-session-id se device_id válido
    let mpResponse;
    try {
    ...
    console.log('[mp-create-payment] 🎉 Payment created successfully:', {
      ...
      '🔒 SECURITY CHECKLIST': {
        ...
        '✅ Issuer ID': paymentData.issuer_id || 'NOT SET', // ✅ NOVO
      ...
      '⚠️ RISK FACTORS': {
        ...
        missing_issuer_id: !paymentData.issuer_id ? '🟡 YES - Pode afetar roteamento' : '✅ NO' // ✅ NOVO
      }
    });
...
| Correção | Descrição |
|----------|-----------|
| **BUILD_ID** | Atualizado para `mp-create-payment@2026-02-04T20:00:00Z-v4` |
| **issuer_id** | Adicionado suporte ao código do emissor (+2 pontos qualidade) |
| **additional_info.payer.address** | Removidos `city` e `federal_unit` (API espera códigos IBGE numéricos) |
| **additional_info.shipments.receiver_address** | Renomeado `city` para `city_name` (campo correto da API) |
| **X-meli-session-id** | Header agora é condicional - só enviado se device_id válido |
| **debug_context** | Adicionados `has_device_id` e `has_issuer_id` para monitoramento |
...
2. [ ] Colar no editor do Supabase Dashboard
3. [ ] Clicar em "Deploy"
4. [ ] Testar pagamento com cartão
5. [ ] Verificar se `build_id` na resposta é `v4`
6. [ ] Verificar se não há erro `SDK_EXCEPTION`
7. [ ] Rodar teste de qualidade no Mercado Pago (espera-se ~100/100)

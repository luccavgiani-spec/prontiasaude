...
export function ServicosSection() {
  ...
  return <section id="servicos" className="py-16 bg-muted/30" style={{ contentVisibility: 'auto', containIntrinsicSize: '800px' }}>
      <div className="container mx-auto px-[25px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {CATALOGO_SERVICOS.map(servico => <ServicoCard key={servico.slug} servico={servico} tipoContratacao={tipoContratacao} descontoContratacao={descontoAtual} showDesconto={descontoAtual > 0} />)}
        </div>
      </div>
    </section>;
}
      ...
      <div className="container mx-auto sm:px-8 lg:px-12 px-[16px] my-[8px]">
        <div className="flex justify-between items-center h-12 lg:h-14">
          ...
          <div className="hidden lg:flex items-center justify-center gap-4 lg:gap-6 flex-1">
            {navItems.map(item => <Link key={item.href} to={item.href} onClick={item.href === "/" ? handleHomeClick : undefined} className={cn("text-xs lg:text-sm font-medium transition-all duration-300 relative group text-center whitespace-nowrap px-2", isActive(item.href) ? "text-primary" : "text-muted-foreground hover:text-primary")}>
                {item.label}
        ...
        {isOpen && <div className="lg:hidden py-4 border-t border-border/50 bg-gradient-to-b from-background to-muted/30 rounded-b-2xl">
            <div className="flex flex-col gap-3">
              {navItems.map(item => <Link key={item.href} to={item.href} onClick={(e) => {
                ...
                }} className={cn("text-sm font-medium transition-all duration-300 px-4 py-2 rounded-xl", isActive(item.href) ? "text-primary bg-primary/10 border border-primary/20" : "text-muted-foreground hover:text-primary hover:bg-primary/5")}>
                  {item.label}
                </Link>)}

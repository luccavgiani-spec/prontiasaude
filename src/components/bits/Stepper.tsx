import React, { useState, Children, useRef, useLayoutEffect, ReactNode, HTMLAttributes } from "react";
...
type StepperProps = HTMLAttributes<HTMLDivElement> & {
  ...
  renderStepIndicator?: (args: { step: number; currentStep: number; onStepClick: (clicked: number) => void }) => ReactNode;
  showFooter?: boolean; // para esconder botões nesta seção
};
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators, }: { step: number; currentStep: number; onClickStep: (clicked: number) => void; disableStepIndicators?: boolean; }) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: "var(--mdb-neutral-900)", color: "var(--mdb-muted)" },
          active: { scale: 1, backgroundColor: "var(--mdb-primary)", color: "var(--mdb-primary)" },
          complete: { scale: 1, backgroundColor: "var(--mdb-primary)", color: "var(--mdb-primary)" },
        }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = { incomplete: { width: 0, backgroundColor: "transparent" }, complete: { width: "100%", backgroundColor: "var(--mdb-primary)" } };
  return <div className="step-connector"><motion.div className="step-connector-inner" variants={lineVariants} initial={false} animate={isComplete ? "complete" : "incomplete"} transition={{ duration: 0.4 }} /></div>;
}

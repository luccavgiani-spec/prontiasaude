...
export default function BlogsIndex() {
  ...
  return <section className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="flex flex-wrap gap-2 mb-8 justify-center">
        {allTags.map(tag => <button key={tag} onClick={() => setActive(tag)} className={`rounded-full border px-4 py-2 text-sm font-medium transition-all ${active === tag ? "bg-primary text-white border-primary" : "bg-white text-neutral-700 border-neutral-200 hover:bg-neutral-50"}`}>
            {tag}
          </button>)}
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map(post => <article key={post.slug} className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="p-6">
              ...
              <div className="mb-4 flex flex-wrap gap-1">
                {post.tags.map(t => <span key={t} className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
                    {t}
                  </span>)}
              </div>
              ...
              <div className="mt-4">
                <Link to={`${basePath}/${post.slug}`} className="inline-flex items-center gap-2 text-primary font-medium hover:underline">
                  Ler artigo completo →
                </Link>
              </div>
            </div>

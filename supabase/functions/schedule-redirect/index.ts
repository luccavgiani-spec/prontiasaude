...
async function registerClickLifePatient(
  ...
  const isDuplicateKey =
    resText.includes("duplicate key") ||
  ...
  if (res.status === 200 || res.status === 201 || res.status === 409 || (res.status === 500 && isDuplicateKey)) {
    ...
    if (!activationRes.ok) {
      ...
      console.warn(
        "[ClickLife] ⚠️ Erro na ativação (não fatal):",
        activationRes.status,
        activationError.substring(0, 200),
      );
      ...
      try {
        ...
        console.warn(
          "[ClickLife] ⚠️ Resposta de ativação não é JSON válido (ok se já ativo):",
          activationText.substring(0, 200),
        );
    ...
    if (!userToken) {
      console.warn(
        "[ClickLife] Login sem token (ok, atendimento usará integrador):",
        JSON.stringify(loginData).substring(0, 100),
      );
      // ✅ NÃO RETORNAR ERRO
      return { success: true };
    }
...
Deno.serve(async (req) => {
  ...
  try {
      ...
      try {
        ...
        if (patientData) {
          ...
          if (!payload.birth_date && patientData.birth_date) {
            ...
            console.warn(
              "[schedule-redirect] ⚠️ birth_date ausente no payload E no banco para:",
              payload.email?.substring(0, 3) + "***",
            );
          }
    ...
    if (payload.force_provider) {
      ...
      console.log(
        `[schedule-redirect] skip_registration=${payload.skip_registration}, plano_ativo=${payload.plano_ativo}`,
      );
...
async function redirectClickLife(payload: SchedulePayload, reason: string, corsHeaders: Record<string, string>) {
  ...
  if (payload.plano_ativo && payload.plan_code) {
    const planoIncluiEspecialistas =
      PLANOS_COM_ESPECIALISTAS.includes(payload.plan_code) || isPlanoEmpresarial(payload.plan_code);
  ...
  console.log(
    `[ClickLife] plano_id selecionado: ${planoId} (plan_code: ${payload.plan_code}, plano_ativo: ${payload.plano_ativo})`,
  );

  // ✅ Garantir que sexo seja 'M' ou 'F'
  const sexoFinal = payload.sexo && (payload.sexo === "M" || payload.sexo === "F") ? payload.sexo : "F";
  ...
  async function tryCreateClickLifeAttendance(): Promise<{
    ok: boolean;
    status: number;
    data?: any;
    errorText?: string;
    ...
    const response = await fetch(`${API_BASE}/atendimentos/atendimentos`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const ORIGINAL_SERVICE_ROLE_KEY =
    Deno.env.get("ORIGINAL_SUPABASE_SERVICE_ROLE_KEY") ?? Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
  const supabaseInstance = createClient(ORIGINAL_SUPABASE_URL, ORIGINAL_SERVICE_ROLE_KEY);
  await saveAppointment(payload, "clicklife", redirectUrl, supabaseInstance);

  // ✅ Gravar métrica de agendamento
  try {
    await supabaseInstance.from("metrics").insert({
...
async function createCommunicarePatient(
  ...
  const PATIENTS_BASE =
    Deno.env.get("COMMUNICARE_PATIENTS_BASE") || "https://api-patients-production.communicare.com.br";
    ...
    try {
      ...
      if (parts.length === 3) {
        ...
        if (!(year === "1990" && month === "01" && day === "01")) {
          ...
          console.log(
            "[Communicare Patients] ✅ Data de nascimento REAL:",
            payload.birth_date,
            "→",
            birthDateFormatted,
          );
  ...
  if (usedBirthDateFallback) {
    console.warn(
      "[Communicare Patients] ⚠️ FALLBACK: Usando data genérica 01011990 (birth_date recebido:",
      payload.birth_date || "NULO",
      ")",
    );
  }
...
async function redirectCommunicare(payload: SchedulePayload, supabase: any, corsHeaders: Record<string, string>) {
  ...
  if (!jwt) {
    ...
    const ssoResponse = await fetch(`${INTEGRATIONS_BASE}/sso/${SSO_CPF}`, {
      method: "GET",
      headers: {
  ...
  if (!patientId) {
    ...
    return new Response(
      JSON.stringify({
        ...
        provider: "communicare",
        error:
          "Não foi possível cadastrar paciente na Communicare. Verifique os dados (nome, CPF, telefone, email) e tente novamente.",
        details: {
  ...
  const queueResponse = await fetch(`${INTEGRATIONS_BASE}/v1/queue`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function getCachedJWT(supabase: any): Promise<string | null> {
  const { data } = await supabase.from("admin_settings").select("value").eq("key", "communicare_jwt").maybeSingle();
